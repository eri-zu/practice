...
export class SceneManager {
  ...
  createText() {
    ...
    const y =
      fontPos.fontBoundingBoxAscent / 2 + fontPos.fontBoundingBoxDescent / 2;
  ...
  getTextDotPos() {
    ...
    // for文2回回して,縦横全てチェックする
    for (let height = 0; height < this.stageHeight; height += density) {
      ++i;
      const slide = i % 2 === 0; //横のfor捜査のスタート位置を決める。 偶数だったらtrue
      width = 0;

      // 全部チェックしてたらあれなので、端っこは見なくてよいようにする

      if (!slide) {
        //別になくてもいいけど、負荷軽減のためにやっておく
        width += 6;
      }

      for (width; width < this.stageWidth; width += density) {
        // https://gyazo.com/110ba5b79cbbb02e76782278ca515284
        // (width + height * this.stageWidth) * 4 + 3でindex番号取得できる
        // index番号で各ピクセルの値取れる
        pixel = imageData[(width + height * this.stageWidth) * 4 + 3]; //alpha値を取得。imageDataはデフォルトで(0, 0, 0, 0)なので文字のところが0以外になるはず。
        if (
          // 文字が描かれてるとこの条件になる
          // alphaが0じゃないということは文字がえががれてる
  ...
  createParticle() {
    this.container = new PIXI.ParticleContainer(this.particlePoints.length, {
      vertices: false,
      position: true,
      rotation: false,
      scale: false,
