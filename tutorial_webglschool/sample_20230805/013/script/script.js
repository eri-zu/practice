...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // テクスチャラッピングのリスト @@@
  const wrapping = [
    'CLAMP_TO_EDGE',   // クランプ（切り捨て）
    'REPEAT',          // 繰り返し
    'MIRRORED_REPEAT', // 反転繰り返し
  ];
  ...
  pane.addBlade({
    view: 'list',
    label: 'wrapping',
    options: wrapping.map((v) => {return {text: v, value: v};}),
    value: wrapping[0],
  })
  .on('change', (v) => {
    app.setTextureWrapping(v.value);
  });
}, false);
...
class App {
  ...
  setTextureWrapping(wrapping) {
    ...
    // 横方向と縦方向にそれぞれ設定できる
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl[wrapping]);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl[wrapping]);
  }
