...
class App {
  ...
  load() {
    return new Promise(async (resolve, reject) => {
      ...
      if (gl == null) {
        ...
        // 答え合わせ用シェーダ
        const vsSourceAnswer = await WebGLUtility.loadFile('./shader/main-answer.vert');
        const vsAnswer = WebGLUtility.createShaderObject(gl, vsSourceAnswer, gl.VERTEX_SHADER);
        const fsSourceAnswer = await WebGLUtility.loadFile('./shader/main-answer.frag');
        const fsAnswer = WebGLUtility.createShaderObject(gl, fsSourceAnswer, gl.FRAGMENT_SHADER);
        this.programAnswer = WebGLUtility.createProgramObject(gl, vsAnswer, fsAnswer);
  ...
  setupLocation() {
    ...
    this.attributeLocationAnswer = [
      gl.getAttribLocation(this.programAnswer, 'position'),
      gl.getAttribLocation(this.programAnswer, 'normal'),
      gl.getAttribLocation(this.programAnswer, 'color'),
    ];
    // attribute のストライド
    ...
    this.uniformLocationAnswer = {
      mMatrix: gl.getUniformLocation(this.programAnswer, 'mMatrix'),
      mvpMatrix: gl.getUniformLocation(this.programAnswer, 'mvpMatrix'),
      normalMatrix: gl.getUniformLocation(this.programAnswer, 'normalMatrix'), // 法線変換行列
      eyePosition: gl.getUniformLocation(this.programAnswer, 'eyePosition'),
    };
  }
  ...
  render() {
    ...
    gl.useProgram(this.programAnswer);
    gl.uniformMatrix4fv(this.uniformLocationAnswer.mMatrix, false, m);
    gl.uniformMatrix4fv(this.uniformLocationAnswer.mvpMatrix, false, mvp);
    gl.uniformMatrix4fv(this.uniformLocationAnswer.normalMatrix, false, normalMatrix);
    gl.uniform3fv(this.uniformLocationAnswer.eyePosition, this.camera.position);
    WebGLUtility.enableBuffer(gl, this.torusVBO, this.attributeLocationAnswer, this.attributeStride, this.torusIBO);
    gl.drawElements(gl.TRIANGLES, this.torusGeometry.index.length, gl.UNSIGNED_SHORT, 0);
  }
}
