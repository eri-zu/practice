...
class App3 {
  ...
  constructor() {
    ...
    // ------------------------------------------------------------------------
    // ★this.renderの中では、thisは常にApp3のインスタンスとする
  ...
  render() {
    // 恒常ループの設定 @@@
    // ★引数から受け取った関数をスクリーンの更新のタイミングに合わせて呼び出してくれる
    // ★requestAnimationFrameは、関数を呼び出す際、グローバルスコープ以下でその関数を実行
    requestAnimationFrame(this.render);

    // コントロールを更新 @@@
    // ★orbit controlは、updateで呼び出すこと必須
