...
uniform float weight[20];      // ガウシアンブラーに使うガウス係数 [20]でlengthが20のuniform変数を受け取るということ（20段階のガウス係数を受け取る） shaderでは配列の長さを可変にできないのでハードコーディングになる

varying vec2 vTexCoord;

void main() {
  vec2 fSize = 1.0 / resolution; // フラグメントあたりの大きさ（1pxのサイズをテクスチャ座標で表した場合）
  vec2 fc = gl_FragCoord.st;     // ピクセルサイズの座標（元々のテクスチャ座標）
