...
// が、その求めたノイズの値が「色ではなくテクスチャ座標」に対して影響するように
// します。これにより、ノイズの模様に応じて描画結果が歪むようになります。
// ============================================================================
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // ノイズの色を可視化するかどうか @@@
  pane.addInput(parameter, 'noiseVisible')
  .on('change', (v) => {
    app.setNoiseVisible(v.value);
  });
  // 時間の経過速度
  pane.addBlade({
    view: 'slider',
  ...
  .on('change', (v) => {
    app.setNoiseDistortion(v.value);
  });
}, false);
...
class App {
   ...
   * @param {number} value - 設定する値
   */
  setNoiseDistortion(value) {
    this.noiseDistortion = value;
  }

  /**
   * ノイズの色を可視化するかどうかを設定する @@@
   * @param {boolean} flag - 設定する値
   */
  setNoiseVisible(flag) {
    this.noiseVisible = flag;
  }
