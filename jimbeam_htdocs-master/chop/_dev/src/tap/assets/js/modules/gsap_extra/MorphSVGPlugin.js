...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(() => {
  var _DEG2RAD = Math.PI / 180,
    ...
    _pathDataToBezier = function (d) {
      let a = (`${d}`).replace(_scientific, (m) => { const n = +m; return (n < 0.0001 && n > -0.0001) ? 0 : n; }).match(_svgPathExp) || [], // some authoring programs spit out very small numbers in scientific notation like "1e-5", so make sure we round that down to 0 first.
        path = [],
        relativeX = 0,
        relativeY = 0,
        elements = a.length,
      ...
      for (i = 0; i < elements; i++) {
        ...
        if (command === 'M') {
          ...
          segment[l++] = relativeX += a[i + 5] * 1;
          segment[l++] = relativeY += a[i + 6] * 1;
    ...
    _createPath = function (e, ignore) {
      let path = _gsScope.document.createElementNS('http://www.w3.org/2000/svg', 'path'),
    ...
    MorphSVGPlugin = _gsScope._gsDefine.plugin({
      ...
      init(target, value, tween, index) {
        ...
        if (shape) {
          ...
          pt = this._addTween(target, 'setAttribute', `${target.getAttribute(p)}`, `${shape}`, 'morphSVG', false, p, (typeof (value.precompile) === 'object') ? (a) => { a[0] = value.precompile[0]; a[1] = value.precompile[1]; } : (p === 'd') ? _buildPathFilter(value.shapeIndex, value.map || MorphSVGPlugin.defaultMap, value.precompile) : _buildPointsFilter(value.shapeIndex));
          if (pt) {
  ...
  MorphSVGPlugin.convertToPath = function (targets, swap) {
    ...
    let a = (!targets || targets.length === 0) ? [] : (targets.length && targets[0] && targets[0].nodeType) ? Array.prototype.slice.call(targets, 0) : [targets],
      i = a.length;
    while (--i > -1) {
  ...
  MorphSVGPlugin.pathDataToBezier = function (data, vars) { // converts SVG path data into an array of {x, y} objects that can be plugged directly into a bezier tween. You can optionally pass in a 2D matrix like [a, b, c, d, tx, ty] containing numbers that should transform each point.
    let bezier = _pathDataToBezier(_parseShape(data, true))[0] || [],
      prefix = 0,
      a,
      i,
      l,
