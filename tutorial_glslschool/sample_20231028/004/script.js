...
class WebGLApp {
  ...
  constructor() {
    ...
    window.addEventListener(
      "pointermove",
      (mouseEvent) => {
        const x = mouseEvent.pageX / window.innerWidth; // 0 - 1の値
        const y = mouseEvent.pageY / window.innerHeight; // 0 - 1の値
        const signedX = x * 2.0 - 1.0; // -1 - 1の値
        const signedY = y * 2.0 - 1.0; // -1 - 1の値
