...
window.addEventListener(
  ...
  async () => {
     ...
     * async/await を使うことで、非同期処理を同期処理のように実行順どおりに記述す
     * ることができます。
     */
...
class WebGLApp {
  ...
  async load() {
    ...
    this.shaderProgram = new ShaderProgram(this.gl, {
      ...
      // ★以下は、attrivute vec3 positionとリンク
      attribute: ["position"],
      stride: [
        3, //  ★vec3だとstrideは3
      ],
  ...
  setupGeometry() {
    ...
    // ※ ShaderProgram.setAttribute で処理するために配列に入れています
  ...
  render() {
    ...
    // ★this.vboを使うことを宣言するみたいなイメージ
    this.shaderProgram.setAttribute(this.vbo);

    // 設定済みの情報を使って、頂点を画面にレンダリングする
    gl.drawArrays(
      gl.POINTS, // ★頂点を描画するプリミティブの形状（点 / 線 / ポリゴン） gl.POINTSは点, gl.LINES, gl.TRIANGLES...
      0, // ★offset（どこからかく？）
      this.position.length / 3 // ★count（何個？）
    );
  }
}
