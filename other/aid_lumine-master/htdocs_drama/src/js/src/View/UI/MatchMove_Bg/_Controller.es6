...
export default class Controller extends Base {
  ...
  rotateDirPGyaku() {
    ...
    tl
      ...
      .add(()=>{
        ...
        // TweenMax.set($('#matches'), {rotationZ: '-=' + 1.5});
        // TweenMax.set(this.$ashiato, {rotationZ: '-=' + 1.5});
  ...
  save() {
    ...
    // TweenMax.set(this.$wrap, {rotationZ: '-=' + 10.5});

    // var $c = this.$wrap.clone();
    // var pos = this.$wrapW.get(0).getBoundingClientRect();
    // TweenMax.set($c, {opacity: .1, 'margin-left': '0px', backgroundColor: 'none'});
