...
void main() {
  ...
  vec2 uvSprite = vec2( 
    u_offset.x + vUv.x * .5, 
    .5 - u_offset.y + vUv.y * .5
  );

  vec4 texture = texture2D( tDiffuse, uvSprite );

    vec3 color = texture.rgb;
    // float alpha = smoothstep(0., .5, texture.r);
    float alpha = texture.a;

  // vec4 cOutput = vec4(1.0, 0.0, 0.0, 0.5);
 //    gl_FragColor = texture + cOutput;
