...
void main( void ) {
  ...
  vec3 baseColor = vec3(1.);
    vec3 color = texture.rgb * baseColor;
    // float alpha = texture.a;
    // float alpha = texture.a * vOpacity * 10.;
    float alpha = texture.a * vOpacity * 3. * ( sin( uTime * vAlphaSpeed * 0.5 + ( 3.14 * 2. * vAlphaSpeed ) ) * 0.5 + 0.5 );
