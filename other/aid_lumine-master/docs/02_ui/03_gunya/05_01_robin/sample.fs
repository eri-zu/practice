...
void main()
{
  vec2 uv = vUv;//gl_FragCoord.xy / res.xy;
  uv -= vec2(1.3, 1.3);
  // uv.x *= vUv.x/vUv.y;
  uv *= 16.0; // Scale the coordinate system
  uv = rotate2d(0.0 * -TWO_PI) * uv;


  // waves
    // vec4 wave_color = vec4(1.0, 1.0, 1.0, 0.0);
  ...
  for(int i = 0; i < 6; ++i) {
    ...
    uv = rotate2d(-0.25 * -TWO_PI) * uv;

    uv.y +=  (sin(sin(uv.x + float(i)*1.0 + ((alpha*0.5) * TWO_PI) )/3.) * 1.6 + (-1.0 * PI));
    wave_width = abs(1.0 / (50.0 * uv.y * glow));
        // wave_color += vec4(wave_width, wave_width, wave_width, wave_width * alpha);
    wave_color += vec3(wave_width, wave_width, wave_width);
  }

    if(wave_color.r <= 0.02)
    {
        discard;
    }
