...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _Controller7 = __webpack_require__(46);
	
	var _Controller8 = _interopRequireDefault(_Controller7);
	
	var _Controller9 = __webpack_require__(50);
	...
	var Controller = function () {
	  ...
	  _createClass(Controller, [{
	    ...
	    value: function setup() {
	      ...
	      this.objs = [new _Controller4.default(3), new _Controller4.default(4), new _Controller4.default(6)];
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var Controller = function () {
	  function Controller() {
	    ...
	    this.colors = [new THREE.Color(0x3afffe), new THREE.Color(0x59ef07), new THREE.Color(0xdb4cff)];
	    this.setup();
...
/***/ (function(module, exports) {
	...
	var Rand_Obj = function () {
	  function Rand_Obj() {
	    ...
	    this.rotation = [Math.PI * .5, Math.PI * (30 / 180), Math.PI * (150 / 180) * -1];
	    this.setup();
	  }
	  ...
	  _createClass(Rand_Obj, [{
	    ...
	    value: function setup() {
	      ...
	      for (var i = 0; i < this.NUM; i++) {
	        var l = this.mesh.children[2 * i + 1];
	        var p1 = [l.geometry.attributes.position.array[0], l.geometry.attributes.position.array[1], l.geometry.attributes.position.array[2]];
	        var p2 = [l.geometry.attributes.position.array[6], l.geometry.attributes.position.array[7], l.geometry.attributes.position.array[8]];
	        var v = [p1[0] - p2[0], p1[1] - p2[1], p1[2] - p2[2]];
	        var nv = [v[0] * -15, v[1] * -15, 0];
	        var p = [nv[0] + p1[0], nv[1] + p1[1], nv[2] + p1[2]];
	        var _geometry = new THREE.Geometry();
	        _geometry.vertices.push(new THREE.Vector3(p1[0], p1[1], p1[2]));
	        _geometry.vertices.push(new THREE.Vector3(p[0], p[1], p[2]));
	        var line = new MeshLine();
	        line.setGeometry(_geometry, function (p) {
	          return _this.LINE_WIDTH / 2;
	        });
	        var material = new MeshLineMaterial({
	          color: this.colors[0],
	          resolution: new THREE.Vector2(window.innerWidth, window.innerHeight),
	    ...
	    value: function show() {
	      ...
	      .to(this.lines[1].material.uniforms.dashOffset, .2, {
	        value: 1,
	        ease: Expo.easeOut
	      }, .0).to(this.lines[1].material.uniforms.dashOffset, .3, {
	        value: 2,
	        ease: Expo.easeOut
	      }, .1).add(function (e) {
	      ...
	      .to(this.lines[0].material.uniforms.dashOffset, .2, {
	        value: 1,
	        ease: Power4.easeOut
	      }, .6 - 0.3).to(this.lines[0].material.uniforms.dashOffset, .3, {
	        value: 2,
	        ease: Power4.easeOut
	      }, .7 - 0.3).add(function (e) {
	    ...
	    value: function resize() {
	      ...
	      this.naisetsu = this.r * 3 / Math.sqrt(3);
	      this.positions = [{ x: -300 * per, y: 0, z: 0 }, { x: -300 * per + this.naisetsu * 1.1, y: this.r * 1.6 * per, z: 0 }, { x: 400 * per, y: -this.r * 1.5 * per, z: 0 }];
	      this.mesh.children.map(function (mesh, index) {
	        if (mesh.name == 'fill') {
	          var scale = _this3.scales[index / 2];
	          var rotation = _this3.rotation[index / 2];
	          mesh.geometry.dispose();
	          mesh.geometry = new THREE.CircleBufferGeometry(_this3.r * scale, _this3.SEGMENT, rotation);
	          mesh.position.set(_this3.positions[index / 2].x, _this3.positions[index / 2].y, _this3.positions[index / 2].z);
	        }
	        if (mesh.name == 'line') {
	          var _geometry2 = new THREE.Geometry();
	          var i = (index - 1) / 2;
	
	          var _posi = _this3.getVertice(_this3.scales[i], _this3.rotation[i]);
	          _posi.map(function (e, i) {
	            var v = i == _posi.length - 1 ? new THREE.Vector3(_posi[0].x, _posi[0].y, 0) : new THREE.Vector3(e.x, e.y, 0);
	            _geometry2.vertices.push(v);
	          });
	          var line = new MeshLine();
	          line.setGeometry(_geometry2, function (p) {
	            return _this3.LINE_WIDTH;
	          });
	          mesh.geometry.dispose();
	          mesh.geometry = line.geometry;
	          mesh.position.set(_this3.positions[i].x, _this3.positions[i].y, _this3.positions[i].z);
	        }
	        if (mesh.name == 'l') {
	          var l = _this3.mesh.children[2 * (index - 6) + 1];
	          var p1 = [l.geometry.attributes.position.array[0], l.geometry.attributes.position.array[1], l.geometry.attributes.position.array[2]];
	          var p2 = [l.geometry.attributes.position.array[6], l.geometry.attributes.position.array[7], l.geometry.attributes.position.array[8]];
	          var v = [p1[0] - p2[0], p1[1] - p2[1], p1[2] - p2[2]];
	          var nv = [v[0] * -15, v[1] * -15, 0];
	          ...
	          _geometry3.vertices.push(new THREE.Vector3(p1[0], p1[1], p1[2]));
	          _geometry3.vertices.push(new THREE.Vector3(p[0], p[1], p[2]));
	          var _line = new MeshLine();
	          _line.setGeometry(_geometry3, function (p) {
	            return _this3.LINE_WIDTH / 2;
	          });
	          mesh.geometry.dispose();
	          mesh.geometry = _line.geometry;
	          mesh.position.set(_this3.positions[index - 6].x, _this3.positions[index - 6].y, _this3.positions[index - 6].z);
	        }
	      });
	    }
	  }]);
