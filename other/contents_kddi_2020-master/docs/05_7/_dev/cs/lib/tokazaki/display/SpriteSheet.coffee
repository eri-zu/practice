...
class SpriteSheet
	...
	addFrameScript:(frame,func)->
		@_frameScripts = @_frameScripts || @_frameScripts = []
		arr = @_frameScripts[frame] || []
		arr.push(func)
		@_frameScripts[frame] = arr
		return
	onUpdateFrame:->
	...
	update:(ct)->
		...
		@_draw()
		if @_frameScripts and @_frameScripts[@_currentFrame]
			arr = @_frameScripts[@_currentFrame]
			for val in arr
				val.call()
		return
	_draw:->
...
class BackgroundSprite extends SpriteSheet
	_draw:null
	constructor:(target,rect,totalFrames,dir,isloop = false,fps = 30)->
	...
	_init:->
		...
		if @_dir is "x"
			...
			@_draw = @_drawXY
			@_iw = @_rect.imgWidth / @_rect.width
			@_ih = @_rect.imgHeight / @_rect.height
			@_totalFrames = @_iw * @_ih
	_drawX:->
		bf = -(@_currentFrame - 1) * @_rect.width
		TrTween.prop(@_target,{backgroundPositionX:bf}).play()
		return
	_drawY:->
		bf = -(@_currentFrame - 1) * @_rect.height
	...
	_drawXY:->
		...
		TrTween.prop(@_target,{backgroundPositionX:bfx,backgroundPositionY:bfy}).play()
		return
